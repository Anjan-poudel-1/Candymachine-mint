...
const mintClick = async (
    ...
    if (!process.env.NEXT_PUBLIC_BUYMARKBEER) {
        buyBeer = false;
        console.log(
            "The Creator does not want to pay for MarkSackerbergs beer 😒"
        );
    }

    try {
        //find the guard by guardToUse.label and set minting to true
        const guardIndex = guardList.findIndex(
            (g) => g.label === guardToUse.label
        );
        if (guardIndex === -1) {
            console.error("guard not found");
            return;
            ...
            <HStack>
                ...
                <Flex justifyContent="flex-end" marginLeft="auto">
                    ...
                    {buttonGuard.startTime > createBigInt(0) &&
                        ...
                        (!buttonGuard.endTime ||
                            solanaTime - buttonGuard.endTime <=
                                createBigInt(0)) && (
                            <>
                                <Text fontSize="sm" marginRight={"2"}>
                                    Starting in:{" "}
                ...
                <VStack>
                    ...
                    >
                        <Button
                            ...
                            isLoading={
                                guardList.find(
                                    (elem) => elem.label === buttonGuard.label
                                )?.minting
                            }
                            loadingText={
                                guardList.find(
